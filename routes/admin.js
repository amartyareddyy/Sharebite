...
router.get("/admin/dashboard", middleware.ensureAdminLoggedIn, async (req,res) => {
	try {
		...
		// Fetch recent pending donations
		const pendingDonations = await Donation.find({ status: "pending" })
			.populate("donor")
			.sort({ createdAt: -1 })
			.limit(5);

		// Fetch recent completed donations with images
		const completedDonations = await Donation.find({ 
			status: { $in: ["collected", "delivered"] },
			collectionImage: { $exists: true, $ne: null }
		})
		.populate("donor")
		.populate("agent")
...
router.get("/admin/donations/previous", middleware.ensureAdminLoggedIn, async (req,res) => {
	...
	{
		console.error('Error fetching previous donations:', err);
		req.flash("error", "Some error occurred on the server.")
		res.redirect("back");
	}
});
