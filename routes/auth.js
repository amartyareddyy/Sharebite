...
router.post("/auth/signup", middleware.ensureNotLoggedIn, async (req,res) => {
	...
	const { firstName, lastName, email, password1, password2, role, drivingLicense } = req.body;
	let errors = [];
	...
	if (role === 'agent' && !drivingLicense) {
		errors.push({ msg: "Driving license number is required for agents" });
	}
	if (role === 'agent' && drivingLicense && !/^[A-Za-z0-9]{8,15}$/.test(drivingLicense)) {
		errors.push({ msg: "Invalid driving license number format. It should be 8-15 alphanumeric characters." });
	}
	if(errors.length > 0) {
		return res.render("auth/signup", {
			title: "User Signup",
			errors, firstName, lastName, email, password1, password2, role, drivingLicense
		});
	}
	...
	try
	{
		...
		if(user)
		{
			...
			return res.render("auth/signup", {
				title: "User Signup",
				firstName, lastName, errors, email, password1, password2, role, drivingLicense
			});
		}
		...
		const newUser = new User({ 
			...
			password: password1, 
			role,
			drivingLicense: role === 'agent' ? drivingLicense : undefined
		});
